...
def thrice
  yield  #thrice.call(5) {x+=1} => 6
  yield  #thrice.call(6) {x+=1} => 7
  yield  #thrice.call(7) {x+=1} => 8
end

#when you call thrice with a block of x+=1,
...
example 8
# twice = -> (x) { 2 * x }          # new & preferred way of writing out lambda
# twice.call(5) # => 10
